...
public class NaviCustomActivity extends BaseActivity implements AMapNaviViewListener{
	...
	// 导航可以设置的参数
	private boolean mDayNightFlag = NavUtils.DAY_MODE;// 默认为白天模式
	private boolean mDeviationFlag = NavUtils.YES_MODE;// 默认进行偏航重算
	private boolean mJamFlag = NavUtils.YES_MODE;// 默认进行拥堵重算
	private boolean mTrafficFlag = NavUtils.OPEN_MODE;// 默认进行交通播报
	private boolean mCameraFlag = NavUtils.OPEN_MODE;// 默认进行摄像头播报
	private boolean mScreenFlag = NavUtils.YES_MODE;// 默认是屏幕常亮
	...
	public void onNaviSetting() {
		...
		bundle.putInt(NavUtils.THEME, mThemeStle);
	...
	private void processBundle(Bundle bundle) {
		...
		if (bundle != null) {
			mDayNightFlag = bundle.getBoolean(NavUtils.DAY_NIGHT_MODE,
					mDayNightFlag);
			mDeviationFlag = bundle.getBoolean(NavUtils.DEVIATION, mDeviationFlag);
			mJamFlag = bundle.getBoolean(NavUtils.JAM, mJamFlag);
			mTrafficFlag = bundle.getBoolean(NavUtils.TRAFFIC, mTrafficFlag);
			mCameraFlag = bundle.getBoolean(NavUtils.CAMERA, mCameraFlag);
			mScreenFlag = bundle.getBoolean(NavUtils.SCREEN, mScreenFlag);
			mThemeStle = bundle.getInt(NavUtils.THEME);
